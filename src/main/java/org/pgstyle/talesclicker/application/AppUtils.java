...
public final class AppUtils {
     ...
     */
    public static void nestedLoop(int outer, int outerLimit, int inner, int innerLimit, BiConsumer<Integer, Integer> action) {
    ...
    public static InputStream getResource(String name) {
        return Optional.ofNullable("/META-INF/org.pgstyle/tales-clicker/" + name)
                       .map(Paths::get).map(Object::toString).map(p -> p.replace("\\", "/"))
                       .map(AppUtils.class::getResourceAsStream).filter(Objects::nonNull)
                       .orElseThrow(() -> new IllegalArgumentException("cannot find resource \"" + name + "\" in application resources"));
    }
    ...
    public static Stream<String> getResources(String path) {
        ...
        int i = 0;
        do {
            files.addAll(AppUtils.listDirectory(path).map((path + (path.endsWith("/") ? "" : "/"))::concat).collect(Collectors.toList()));
    ...
    public static Stream<String> listDirectory(String path) {
        List<String> files = new ArrayList<>();
        InputStream is = AppUtils.class.getResourceAsStream("/META-INF/org.pgstyle/tales-clicker/" + path);
    ...
    public static Class<?> getCallerClass(int depth) {
        try {return Class.forName(Thread.currentThread().getStackTrace()[depth + 3].getClassName());}
        catch (ReflectiveOperationException | ArrayIndexOutOfBoundsException e) {return null;}
    }
