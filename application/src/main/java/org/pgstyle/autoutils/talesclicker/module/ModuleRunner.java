...
public final class ModuleRunner extends Thread {
    ...
    private static final long RETRY_TIMEOUT = Module.calculateTimeout(AppConfig.getConfig().getModulePropertyAsReal("manager", "retry.frequency"));
    ...
    public static ModuleRunner of(Class<? extends Module> module, Environment env, String[] args) {
        try {
            ...
            Console.log(Level.ERROR, "failed to instantiate module: %s", e);
            e.printStackTrace();
            return null;
        }
    }
    ...
    private ModuleRunner(Module module, Environment env, String[] args) {
        ...
        Console.log(Level.DEBUG, "create module runner [%s]", this.getName());
        if (args.length > 0) {
            Console.log(Level.DEBUG, "with arguments: %s", Arrays.toString(args));
        }
    ...
    public void shutdown(Signal signal) {
        if (this.isAlive()) {
            Console.log(Level.INFO, "runner [%s] received shutdown signal: %s", this.getName(), signal);
            synchronized (this.module) {
    ...
    @Override
    public void run() {
        Console.log(Level.DEBUG, "started module runner [%s]", this.getName());
        this.state = State.RUNNING;
        // initialise module
        boolean runnable = false;
        try {
            Console.log(Level.TRACE, "%s - start initialise", this.module.getClass().getSimpleName());
            runnable = this.module.initialise(this.env, this.args);
            Console.log(Level.TRACE, "%s - finish initialise", this.module.getClass().getSimpleName());
        }
        catch (RuntimeException e) {
            Console.log(Level.ERROR, "error when initialising module: %s", e);
            e.printStackTrace();
        }
        ...
        while (runnable) {
            ...
            if (control.getAction() == Action.RELOAD) {
                Console.log(Level.DEBUG, "reload module runner [%s]", this.getName());
                Console.log(Level.TRACE, "%s - start reload/initialise", this.module.getClass().getSimpleName());
                control = Optional.ofNullable(this.reload(Optional.ofNullable(control.getArgs()).orElse(this.args))).orElse(control);
                Console.log(Level.TRACE, "%s - finish reload/initialise", this.module.getClass().getSimpleName());
            }
            // Action: stop, terminate
            if (control.getAction() != Action.NEXT && control.getAction() != Action.RELOAD) {
                Console.log(Level.DEBUG, "stop module runner [%s]", this.getName());
                this.state = State.STOP;
                runnable = false;
            }
        }
        // stop and finalise module
        Console.log(Level.DEBUG, "finalise module runner [%s]", this.getName());
        boolean finalise = false;
        try {
            Console.log(Level.TRACE, "%s - start finalise", this.module.getClass().getSimpleName());
            ...
            Console.log(Level.TRACE, "%s - finish finalise", this.module.getClass().getSimpleName());
        }
        catch (RuntimeException e) {
            Console.log(Level.ERROR, "error when finalising module: %s", e);
            e.printStackTrace();
        }
        if (!finalise) {
        ...
        Console.log(Level.DEBUG, "runner [%s] exited with state: %s", this.getName(), control.getSignal());
    ...
    private ModuleControl reload(String[] args) {
        try {
            ...
            Console.log(Level.ERROR, "error when reloading module: %s", e);
            e.printStackTrace();
    ...
    private ModuleControl execute() {
        ...
        try {
            Console.log(Level.TRACE, "%s - start execute", this.module.getClass().getSimpleName());
            control = this.module.execute();
            Console.log(Level.TRACE, "%s - finish execute", this.module.getClass().getSimpleName());
        }
        catch (RuntimeException e) {
            Console.log(Level.ERROR, "error when executing module, %s", e);
            e.printStackTrace();
        }
        if (Objects.isNull(control) || control.getDelay() < 0) {
            Console.log(Level.ERROR, "illegal module control: %s", control);
        ...
        try {
            ...
            Console.log(Level.ERROR, "interrupted: %s", e);
            e.printStackTrace();
